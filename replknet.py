...
class RepLKNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
        ...
        super().__init__()
        self.mean = torch.as_tensor([0.485,0.456,0.406])[None,:,None,None]
        self.std = torch.as_tensor([0.229,0.224,0.225])[None,:,None,None]
        ...
        for stage_idx in range(self.num_stages):
            ...
            if stage_idx < len(layers) - 1:
                ...
                transition = nn.Sequential(
                    ...
                    conv_bn_relu(channels[stage_idx + 1], channels[stage_idx + 1], 3, stride=s, padding=1, groups=channels[stage_idx + 1]))
    ...
    def forward_features(self, x):
        ...
        for stem_layer in self.stem[0:3]:
            if self.use_checkpoint:
        ...
        if self.out_indices is None:
            ...
            for stage_idx in range(self.num_stages):
                ...
                x = self.stages[stage_idx](x, stage_idx)
                # out.append(x)
...
def create_RepLKNet31B(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,3], channels=[128,256,512,1024],
...
if __name__ == '__main__':
    ...
    decoder_path = './RepLKNet-31B_ImageNet-1K_224.pth'
    model.load_state_dict(torch.load(decoder_path, map_location='cuda:0'), strict=True)
    model.eval()
